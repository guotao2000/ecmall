 ...
 *
 */
$header = array('Content-Type: application/atom+xml', $acc_req->to_header('http://'.$_SERVER['HTTP_HOST']));
...
if (preg_match('%<name>(.*)</name>%sm', $result)) {
	preg_match('%<name>(.*)</name>%sm', $result, $matches);
	$go_url="/index.php?app=third_login&act=douban_callback&douban_token="
	.$token."&douban_token_secret=".$token_secret."&douban_nickname=".urlencode($matches[1]);
