...
class GcategoryModel extends BaseModel
{
   ...
   /**
     ...
     * @return array
     */
    function get_listyxw($parent_id = -1,$store_id=0, $shown=0)
    {
        if ($parent_id >= 0)
        {
            return $this->find(array(
                'conditions' => "parent_id = '$parent_id' and store_id='$store_id' and if_show='$shown'",
                'order' => 'sort_order, cate_id',
            ));
        }
        else
     ...
     * 取得options，用于下拉列表
     */
    function get_optionsyxw($parent_id = 0,$store_id=0, $shown = 0)
    {
        $res = array();
        $regions = $this->get_listyxw($parent_id,$store_id, $shown);
        foreach ($regions as $region)
        {
            $res[$region['cate_id']] = $region['cate_name'];
        }
        return $res;
    }
