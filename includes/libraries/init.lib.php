...
$price = $price +$val['price'] * $val['quantity'];
		...
		$coupon = $model_cart->getAll("SELECT *FROM ".DB_PREFIX."coupon_sn couponsn ".
			...
			"WHERE (coupon.stores_allow like '%,".$store_id .",%' or coupon.stores_allow='0')  AND couponsn.remain_times >=1 ".
"AND user_coupon.user_id=".$this->visitor->get('user_id')." ".
"AND coupon.start_time <=UNIX_TIMESTAMP(NOW())  AND coupon.end_time >= UNIX_TIMESTAMP(NOW()) AND coupon.min_amount <= ".$price." AND if_issue=1"	
);
return $coupon;
}
}
