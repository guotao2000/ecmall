...
class CashierApp extends ShoppingbaseApp
{
    ...
    function index()
    {
       ...
       // {
		...
		$sql_pay="select * from ecm_payment where store_id=$seller_id and enabled=1 order by sort_order desc";
		$wap_payments=$payment_model->getAll($sql_pay);
	...
	function jiesheng()
	{
        ...
        /* 内部根据订单号计算节省的钱数 */
        $order_model =& m('order');
		$order_sql="SELECT * from ecm_order where order_id=".$order_id;
		$goods_sql="SELECT a.price,a.shichang,b.quantity from ecm_goods_spec a LEFT JOIN ecm_order_goods b on a.spec_id=b.spec_id where b.order_id=".$order_id." and a.spec_id in(SELECT spec_id from ecm_order_goods where order_id=".$order_id.")
";
        $order_rows=$order_model->getAll($order_sql);
		if(!count($order_rows))
		{
			$this->show_warning('订单不存在，非法输入');

            return;
		}
        $order_amount=$order_rows[0]['order_amount'];
		$rows=$order_model->getAll($goods_sql);
		$sum_shichang=0;
		$sum_price=0;
		foreach($rows as $row)
		{
			$sum_shichang+=$row['shichang']*$row['quantity'];
		}
		$sum_price=$sum_shichang-$order_amount;
		if($sum_price<0.5*$order_amount)
		{
			$sum_price=0.5*$order_amount;
			$sum_shichang=1.5*$order_amount;
		}
		//$discount=$order_rows[0]['discount'];
		$this->assign("store_id",$order_rows[0]['seller_id']);
		$this->assign("sum_shichang",$sum_shichang);
		$this->assign("order_amount",$order_amount);
		$this->assign("sum_price",$sum_price);
		$this->assign("shipfee",$order_rows[0]['shipping_fee']);
		include 'weixinyxw.php';
	    $jssdk = new JSSDK("wx64794ef985549154", "5dcdf5cdbc6e9e1ff23c39836df9e236");
        $signPackage = $jssdk->GetSignPackage();
	    $this->assign('signPackage', $signPackage);
		$this->display("cashier.success.html");
