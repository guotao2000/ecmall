...
/*结束*/
    ...
    function login()
    {
		...
		if(isset($_GET['openid'])){
			$mod_member = &m('member');
			$parentid = $mod_member->db->getOne("select parentid from ecm_member where user_name='" . $_GET['openid'] . "'");
			
			if(isset($_COOKIE['uin'])){
				setcookie('uin', '', gmtime()-3600);
				
			}

			//将推荐号存入cookie中
			//setcookie('uin', $parentid);
			setcookie("uin",$parentid,time()+31622400);
        ...
        if ($this->visitor->has_login)
        {
			if($ss==2)
			{
                ...
                header("Location:index.php?app=hongbao"); 
				return;
			}
			...
			{
				/*$this->show_message('您已经登录了',
				'back_before_register', 'index.php?app=weixin_view&wx_id=38&wxtw_id=50&openid=' . $_GET['openid']
				);*/
                header("Location:index.php?app=weixin_view&wx_id=38&wxtw_id=50&openid=" . $_GET['openid']); 
				return;
			}
            if($ss==5)
            {
              
                header("Location:index.php?app=weixin_hb&wx_id=38&wxtw_id=50&openid=" . $_GET['openid']); 
                return;
            }
            if($ss==6)
            {
              
                header("Location:index.php?app=zhuanti&id=11&uin=183&act=index&openid=" . $_GET['openid']); 
                return;
            }
           ...
           /* $this->show_message('您已经登录了',
            ...
            header("Location:index.php?app=default&flag=1"); 
			return;
        }
        ...
        if (!IS_POST)
        {
			...
			if(!empty($openid)){
				...
				if($is_mob){
					...
					{
						//$this->show_message('',
						//'back_before_register', 'index.php?app=weixin_view&wx_id=38&wxtw_id=50&openid=' . $openid
						//);
						 header('Location:index.php?app=weixin_view&wx_id=38&wxtw_id=50&openid=' . $openid);
						return;
					}
        ...
        else
        {
			...
			setcookie("user_id",$user_id,time()+31622400);
			$this->_do_login($user_id);
...
class StorebaseApp extends FrontendApp
{
    ...
    function get_store_data()
    {
        ...
        if ($store === false)
        {
            ...
            if ($store['state'] == 2)
            {
				header('Location:index.php?app=storeyxw&id=8805&status=0');
