...
class My_favoriteApp extends MemberbaseApp
{
    ...
    function _list_collect_store()
    {
		...
		//统计每个店铺商品数量
		$db = &db();
		foreach($collect_store as $key => $val){
			$store_id = $collect_store[$key]['store_id'];
			$sql = "select count(a.goods_id) as goods_num from ecm_goods as a LEFT JOIN ecm_store as b ON a.store_id = b.store_id where b.store_id=" . $store_id;
			$result = $db->getOne($sql);
			$collect_store[$key]['goods_num'] = $result;
		}
