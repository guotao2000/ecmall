...
class Weixin_viewApp extends MallbaseApp
{
    var $_wxtuwen_mod;
    ...
    function index()
    {
		...
		$this->display('user_hongbao-Get.html');
    }
	...
	function get_hongbao(){
		...
		if(count($results_coupon) > 0){
			...
			//检测该用户是否已抢过红包
			$res_arr = $mod_member->db->getAll("select a.coupon_id from ecm_coupon_sn a left join ecm_user_coupon b on a.coupon_sn=b.coupon_sn LEFT JOIN ecm_coupon c on c.coupon_id=a.coupon_id where b.user_id=" . $user_id . " and c.coupon_name='抢红包'");
