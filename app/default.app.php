...
class DefaultApp extends MallbaseApp
{
	function index(){
		...
		if($this->visitor->has_login)
		{
			...
			$sql_addr="SELECT * from ecm_address where user_id=".$user_id ." and `enable`=1 and store_id is not NULL";
			$db=&db();
			$rows_addr=$db->getAll($sql_addr);
			if(count($rows_addr))
			{
				header("Location:/index.php?app=storeyxw&status=1&id=".$rows_addr[0]['store_id'].""); 
				exit;
			}
	...
	function get_store()
	{
		...
		$url="http://api.map.baidu.com/geocoder/v2/?address=".urlencode($address_name.$address_name2)."&output=json&ak=2667f496b45f6c9c4e64e6cd8f0344ed";
