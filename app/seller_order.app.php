...
class Seller_orderApp extends StoreadminbaseApp
{
	...
	function received_pay(){
		...
		{
			$model_order    =&  m('order');
			$string_sql="select * from ecm_order where order_id=".$order_id;
			$o_rows=$model_order->db->getAll($string_sql);
			if($o_rows[0]['status']==21){
			  $model_order->edit(intval($order_id), array('status' => 13, 'pay_time' => gmtime()));
			}
			if($o_rows[0]['status']==30){
			 }
