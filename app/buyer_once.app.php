...
include 'jssdk.php';
/* 一键登录控制器 */
class Buyer_onceApp extends StorebaseApp
{
    var $buyer_once_mod;
	var $_goods_mod;
	...
	function Buyer_onceApp()
    {
        parent::__construct();
		$this->buyer_once_mod = & m('goodsqa');
...
where a.goods_id={$id} and a.state=3 and (unix_timestamp() between a.start_time and a.end_time)";
		...
		$this->assign('signPackage', $signPackage);
			...
			$this->display('buyer_once.index.html');
	...
	function register (){
		...
		if(IS_POST){
			...
			//dump($_POST);
			$ms =& ms(); //连接用户中心
			$user_name = trim($_POST['consignee']);
			...
			if(empty($user_id))
			{
				...
				if(empty($user_id)){
					...
					$db->query("update ecm_address set enable=0 where phone_mob=".$mobile);	
				}
			}
			else{
				$db->query("update ecm_address set enable=0 where phone_mob=".$mobile);	
			...
			//保存地址
			$sql = "insert into ecm_address (user_id,consignee,region_id,region_name,address,phone_mob,enable) values ('{$user_id}','{$user_name}','{$region_id}','{$region_name}','{$address}','{$mobile}',1)";
			$db = &db();
			$db->query($sql);
			...
			foreach($malls as $val)
			{
				if($store_id==$val)
				{
					$this->display('address.add1.html');
					exit;
					//break;
				}
			}
