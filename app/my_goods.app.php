...
class My_goodsApp extends StoreadminbaseApp
{
    ...
    function My_goodsApp()
    {
		...
		$this->_seckill_configs = file_exists(ROOT_PATH."/data/seckill.inc.php") ? include(ROOT_PATH."/data/seckill.inc.php") : '';   //新增
    ...
    function index()
    {
	 ...
	 //by yangxiuwei $this->assign('sgcategories', $this->_get_mgcategory_options());
	  /* 取得商品分类 */
        $gcategories = $this->_get_options();
	 $this->assign('sgcategories', $gcategories);
    ...
    function _get_goods($conditions, &$page)
    {
		...
		$goods_list = $this->_goods_mod->get_listyxw(array(
            'conditions' => $conditions,
            'count' => $count,
            'order' => "$sort $order",
            'limit' => $limit,
    ...
    function _get_conditions()
    {
        ...
        if (trim($_GET['keyword']))
        {
            ...
            $conditions .= " AND (goods_name {$str} OR brand {$str} OR cate_name {$str} OR gs.sku {$str} )";
        }
        if ($_GET['character'])
        {
		...
		if ($_GET['sgcate_id']!=0)
        {
			$stryxw=$_GET['sgcate_id'];
			$conditions .= " AND (cate_id_1=".$stryxw." OR cate_id_2=".$stryxw." OR cate_id_3 =".$stryxw." OR cate_id_4 =".$stryxw." )";
		}
      
        if(intval($_GET['character'])==10)
        {
            $conditions .=" AND gs.sku like '%".$_GET['keyword']."%'";
        }
    ...
    function add()
    {
        ...
        if (!IS_POST)
        {
             ...
             $uploadfiles = $this->_uploadedfile_mod->find(array(
                 ...
                 'conditions' => "belong=".BELONG_GOODS." AND item_id=0 AND store_id=".$this->_store_id." and sid='".SESS_ID."'",
                 'order' => 'add_time ASC'
             ));
   ...
   function ajax_col()
   {
        ...
        switch ($goods['spec_qty'])
        {
            case 0: // 没有规格
                $specs[intval($_POST['spec_id'])] = array(
                    ...
                    'shichang' => $this->_filter_price($_POST['shichang']),
                    ...
                    if ($spec_1)
                    {
                        if (($spec_id = intval($_POST['spec_id'][$key]))) // 已有规格ID的
                        {
                            $specs[$key] = array(
                                ...
                                'shichang'  => $this->_filter_price($_POST['shichang'][$key]),
	...
	function quanzhong()
	{
		$int_goodid=isset($_GET['good_id'])?intval($_GET['good_id']):0;
		$sql="update ecm_goods set quanzhong=unix_timestamp(  ) where goods_id= ".$int_goodid;
		if($int_goodid>0)
		{
			$db=&db();
			echo "".$db->query($sql);
	...
	function jiangquan()
	{
		...
		$sql="update ecm_goods set  quanzhong=if((quanzhong is null or quanzhong<0),0,quanzhong-86400) where goods_id= ".$int_goodid;
		if($int_goodid>0)
		{
			$db=&db();
			echo "".$db->query($sql);
