...
class CartApp extends MallbaseApp
{
	...
	function kx_price($spec_info)
	{
		...
		$sql="select kuaixun_state,kuaixun_id,kuaixun_price from  ecm_kuaixun_promotion where (unix_timestamp() between  start_time and end_time ) and  FIND_IN_SET('".$spec_info['store_id']."',store_ids) and  kuaixun_state in (2,3) and goods_id=".$spec_info['goods_id'];
		...
		if(!empty($kx_state)){
			if($kx_state['kuaixun_state']==2)
			{
			   $sql_up="update ecm_kuaixun_promotion set kuaixun_state=3 where  (unix_timestamp() between  start_time and end_time ) and  FIND_IN_SET('".$spec_info['store_id']."',store_ids)";
			   $db->query($sql_up);
			   return $kx_state['kuaixun_price'];
			}
			else if($kx_state['kuaixun_state']==3){
			   return $kx_state['kuaixun_price'];
			}
			else 
			{
