...
class OrderApp extends ShoppingbaseApp
{
    ...
    function index()
    {
        ...
        if (!IS_POST)
        {
		  ...
		  $form['data']['shipping_methods']=$this->_get_shipping($store_id,$res_info['region_id']);
			...
			$form['data']['totalcount']=$db->getOne($sql_count);
			$sql_pay="select * from ecm_payment where store_id=$store_id and enabled=1 order by sort_order desc";
		   $wap_payments=$db->getAll($sql_pay);
		   // $this->assign('wappayments', $wap_payments);
			$form['data']['wappayments']=$wap_payments;
        ...
        else
        {
            ...
            $order_id = $order_type->submit_order(array(
                ...
                'uin'           =>  $_COOKIE["uin"],
				'o_from'           =>  "手机网站",
				'postscript'           =>  $_POST['remark'],
				'coupon_sn'=>isset($_POST['coupon_sn'])?$_POST['coupon_sn']:"",
            ));
            $user_id = $this->visitor->get('user_id');
			$sql_addr="SELECT * from ecm_address where enable=1 and user_id=". $user_id;
			$db=&db();
			$rows_addr=$db->getAll($sql_addr);
			if(count($rows_addr))
			{
				$addr_update="update  ecm_address set store_id=".$store_id." where addr_id=".$rows_addr[0]['addr_id'];
				$db->query($addr_update);
				$addr_update="update  ecm_order_extm set addr_id=".$rows_addr[0]['addr_id']." where order_id=".$order_id;
				$db->query($addr_update);
	...
	function pay_yxw($order_id)
	{
		$SQL="select * from ecm_order where order_id=".$order_id;
		$db=&db();
		$rows=$db->getAll($SQL);
		if(count($rows))
		{
		   if(intval($_REQUEST['payment'])>0)
			{//trade_sn  order_sn
			   header("Location:index.php?app=bqds&act=gopay_yxw&trade_sn=".$rows[0]['order_sn']."&order_id=".$order_id."&payment_id=".intval($_REQUEST['payment'])."");
			   exit;
			}else{
			   header('Location:index.php?app=cashier&trade_sn='.$rows[0]['order_sn'].'&order_id=' . $order_id);
			   exit;
			}
		}else{
			$this->show_warning("输入非法");
             exit;
		}
            ...
            default:
				...
				for($i=0;$i<count($cart_items);$i++)
				{
				   ...
				   if(empty($sku))
					...
					$sql_str = "select a.price from ecm_promotion_goods a LEFT JOIN ecm_promotion b on a.p_id=b.p_id 
where UNIX_TIMESTAMP() BETWEEN b.start_time and b.end_time and state=3 and a.goods_sn=".$sku." and a.store_id=".$cart_items[$keys[$i]]['store_id']." and b.p_type=1";
					$result = $cart_model->db->getAll($sql_str);

					if(count($result) > 0){
                        $result=$result[0];
						$cart_items[$keys[$i]]['price']=$result['price'];
						
						
					}else
                    {
                        $sql_spec="SELECT price from ecm_goods_spec where spec_id=".$cart_items[$keys[$i]]['spec_id'];
                        $price= $cart_items[$keys[$i]]['price'];
                        $cart_items[$keys[$i]]['price']=empty($price)?$cart_items[$keys[$i]]['price']:$price;
                    }
                 ...
                 foreach ($cart_items as $rec_id => $goods)
                {
				 ...
				 $str_manjian="select * from ecm_promotion where p_type=3 and state=3 and (unix_timestamp() between  start_time and end_time ) and FIND_IN_SET(".$store_id.",stores) order by weight desc";
					$rowsmj=$store_model->db->getAll($str_manjian);
					foreach ($rowsmj as $key => $value) {
						$arr_cuxiaoidsc[]=$value['p_id'];//满赠
					}
				if(count($string_cuxiaoids)>0||count($arr_cuxiaoidsc)>0){
				
                  //$return['amount']=$this->cuxiao_total($store_id);/////////////////////////////////////////////////////////////////
                  require_once($_SERVER['DOCUMENT_ROOT'] ."/extend/promotion.php");
                   $_pro=new promotion; 
                   $return['amount']=$_pro->Do_total($store_id);
   ...
   //查找购物车指定商品的数量
	...
	function _get_shipping($store_id,$region_id)
	{
		...
		//print_r($rows);
		//exit();
		$arr_keys=array_keys($rows);
		for($i=0;$i<count($arr_keys);$i++)
		{
			$j=$arr_keys[$i];
			$regions=$rows[$j]['cod_regions'];
			//echo $regions;
					
		    $rows[$j]['cod_regions']=implode(",",$regions);
	...
	function coupon()
	{
		...
		$sql_coupon="select a.*,coupon_sn from ecm_coupon a left join ecm_coupon_sn b on a.coupon_id=b.coupon_id   where UNIX_TIMESTAMP(now()) BETWEEN a.start_time and  a.end_time and a.if_issue=1 and coupon_sn=".$coupon_id;//stores_allow
		$db=&db();
		//$this->mod_carts= & m('coupon');
		$row_coupons=$db->getAll($sql_coupon);
		
		if(!count($row_coupons))
		{
			$result['result']=0;
			$result['status']=1;
			$this->json_result($result,"校验失败！");
		...
		{
			$row_coupon=$row_coupons[0];
			//print_r($row_coupon);
			$arr_sids=array_filter(explode(",",$row_coupon['stores_allow']));
			$arrkeys=array_keys($arr_sids);
			
			for($i=0;$i<count($arrkeys);$i++)
			{
				
				if($arr_sids[$arrkeys[$i]]==$store_id)
				{
					break;
				}
				if(trim($arr_sids[$arrkeys[$i]])=='0')
				{
					break;
				}
				if($i==count($arrkeys)-1)
				{
					$result['result']=0;
					$result['status']=2;
					$this->json_result($result,"校验失败！-".count($arrkeys));
					exit();
				}
			}
		}
		...
		foreach($rows_goods as $kc=> $vc)
		{
			$rows_goods[$kc]['cate_noallow']=$row_coupon['cate_noallow'];
		}
		...
		if(!count($rows_goods))
		{
			$result['result']=0;
			$result['status']=3;
			$this->json_result($result,$row_coupon['remark']);
			exit();
		}else
		{
	...
	function mycart_coupon($v)
	{
		...
		$coupon_cids=array_filter(explode(",",$v['cate_noallow']));
	...
	function shipping()
	{
		...
		/*$sql_cart="select b.*,a.price cprice,a.quantity,a.remains from ecm_cart a left join ecm_goods b on a.goods_id=b.goods_id where a.session_id='". SESS_ID ."' and a.store_id= ".$store_id;
		//$this->cuxiao_total($store_id);
		$rows_goods=$db->getAll($sql_cart);
