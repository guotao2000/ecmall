...
class OrderApp extends BackendApp
{
    ...
    function index()
    {
        ...
        if(strstr($conditions,'buyer_name'))
        {
            $conditions.="  or buyer_id in (SELECT user_id  from ecm_member where wx_nickname LIKE '%".$_GET['search_name']."%' ) ";
        }
    ...
    function turn()
    {
        ...
        if (!$order_id)
        {
         ...
         if (!IS_POST){
            ...
            if(empty($_POST['seller_id']) || empty($_POST['seller_name'])){
                $this->show_warning('店铺号或者店铺名称为空！');
                exit();
            }
             $order = & m('order');
             $data = array(
             ...
             if($order->edit($order_id,$data)){
                $this->show_message('甩单成功',
