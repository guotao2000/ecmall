...
class GoodsApp extends StorebaseApp
{
    ...
    //显示商品图文详情
    function tuwen(){
		$id = empty($_GET['spec_id']) ? 0 : intval($_GET['spec_id']);
        if (!$id)
        {
            $this->show_warning('Hacking Attempt');
            return;
		...
		$mod_goods = &m('goods');
		$goods_id = $mod_goods->db->getOne("select goods_id from ecm_goods_spec where spec_id=" . $id);

        /* 可缓存数据 */
		$data = $this->_get_common_info($goods_id);
        if ($data === false)
        {
            return;
        }
    ...
    function _get_common_info($id)
    {
        ...
        if ($data === false)
        {
			...
			$db = &db();
			$sql="select state kuaixun_state,p_id kuaixun_id,price kuaixun_price from  ecm_kuaixun_promotion_v where (unix_timestamp() between  start_time and end_time ) and  FIND_IN_SET('".$goods['store_id']."',store_id) and  state in (3) and goods_id=".$goods['goods_id'];
			$result = $db->getAll($sql);

			//print_r($result);
			//echo $goods['store_id'] . '--' . $goods['goods_id'];
			//exit;
			...
			if (!$goods || $goods['closed'] == 1 || $goods['state'] != 1)
            {
                $this->show_warning('goods_not_exist');
                return false;
            }
