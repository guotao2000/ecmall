...
<script type="text/javascript">
    $(function () {
        ...
        var a=  $('#mySelectyxw').children('option:first');
          ...
          $("#region>select").eq(0).hide();
          // $("#region>select").eq(1).hide();
           $("#region").change(function(){
           
           
            if( $("#region>select").size()>2)
            {
            
             $("#region>select").eq(4).hide();
            }
           });
...
<div class="page-role good-page">	
  ...
  <link rel="stylesheet" type="text/css" href="<?php echo $this->res_base . "/" . 'bqmart/template/css/order/order.css'; ?>" >	
     ...
     <div class="pxui-area">
        ...
        <form action="index.php?app=buyer_once&act=register" method="post" id="editAddressForm">
...
?>
    ...
    font-family: 微软雅黑;font-size: 16px;padding: 8px 30%; margin:20px 0px;" name="submit1" value="保&nbsp;&nbsp;存"	/>
		 ...
		 <input type="hidden" name="cart_id" value="<?php echo $this->_var['cart_id']; ?>" />
       </form>
