...
function update_copyed_goods($arrGoodsData){
	...
	//更新商品表
	$db->query("UPDATE ". DB_PREFIX ."goods SET default_image=REPLACE(default_image,'{$replace_from}','{$replace_to}'),description=REPLACE(description,'{$replace_from}','{$replace_to}') where goods_id='{$gid}'");
//更新上传文件表
	$db->query("INSERT INTO ". DB_PREFIX ."uploaded_file(store_id,file_type,file_size,file_name,file_path,add_time,belong,item_id) SELECT '{$sid}',file_type,file_size,file_name,replace(file_path,substring(file_path, locate('/store_', file_path)+7,locate('/goods_', file_path)-locate('/store_', file_path)-7),'{$sid}'),add_time,belong,{$gid} FROM " . DB_PREFIX ."uploaded_file where item_id='{$old_gid}'");
	...
	foreach($imgs as $v){
		...
		$new_img=str_replace($replace_from,$replace_to,$v['image_url']);
		$new_thumb=str_replace($replace_from,$replace_to,$v['thumbnail']);
