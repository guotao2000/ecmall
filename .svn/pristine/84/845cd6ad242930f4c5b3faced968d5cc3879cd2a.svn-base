...
class GcategoryApp extends BackendApp
{
    ...
    function add()
    {
        ...
        {
            //上传文件
			if(is_uploaded_file($_FILES["cate_picurl"]["tmp_name"])){
				$upfile=$_FILES["cate_picurl"];
				...
				if($ok == 1 && $error == 0){
					//检测目录
					if(!is_dir($_SERVER['DOCUMENT_ROOT'] . '/data/cate_pic')){
						mkdir($_SERVER['DOCUMENT_ROOT'] . '/data/cate_pic');
					}
				
					$flag = move_uploaded_file($tmp_name, $_SERVER['DOCUMENT_ROOT'] . "/data/cate_pic/" . $name);
					if($flag){
						//$picurl = SITE_URL . "/data/cate_pic/" . $name;
						$picurl = "/data/cate_pic/" . $name;
					}else{
    ...
    function edit()
    {
        ...
        else
        {
			...
			if(is_uploaded_file($_FILES["cate_picurl"]["tmp_name"])){
				...
				if($ok == 1 && $error == 0){
					...
					if($flag){
						//$picurl = SITE_URL . "/data/cate_pic/" . $name;
						$picurl = "/data/cate_pic/" . $name;
						//chmod($_SERVER['DOCUMENT_ROOT'] . $_POST['cate_picurl_hide'], 0755);
						//unlink($_SERVER['DOCUMENT_ROOT'] . $_POST['cate_picurl_hide']);
					}else{
						$this->show_warning('图片上传失败！');
						return;						
					}
			...
			$data = array(
				'cate_name'  => $_POST['cate_name'],
				'parent_id'  => $_POST['parent_id'],
				'cate_picurl' => empty($picurl)? trim($_POST['cate_picurl_hide']):trim($picurl),
				'sort_order' => $_POST['sort_order'],
				'if_show'    => $_POST['if_show'],
				);
    ...
    function drop()
    {
		...
		/*for($i=0; $i<count($ids); $i++){
			$cate_picurl = $mod_gcategory->db->getOne("select cate_picurl from ecm_gcategory where cate_id=" . $ids[$i]);
			chmod($_SERVER['DOCUMENT_ROOT'] . $cate_picurl, 0755);
			unlink($_SERVER['DOCUMENT_ROOT'] . $cate_picurl);
		}*/
