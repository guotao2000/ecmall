...
class GoodsApp extends StorebaseApp
{
    ...
    function _get_common_info($id)
    {
        ...
        if ($data === false)
        {
			...
			$db = &db();
			$sql="select kuaixun_state,kuaixun_id,kuaixun_price from  ecm_kuaixun_promotion where (unix_timestamp() between  start_time and end_time ) and  FIND_IN_SET('".$goods['store_id']."',store_ids) and  kuaixun_state in (3) and goods_id=".$goods['goods_id'];
