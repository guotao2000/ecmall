        ...
        switch ($object->Event)
        {
            case "subscribe":
				...
				if(count($default_arr) > 0){
					$content[] = array("Title"=>$default_arr['title'], "Description"=>$default_arr['description'], "PicUrl"=>$default_arr['picurl'], "Url" =>$default_arr['url']);
				} else {
				...
				if(count($result_arr) > 0){
					foreach($result_arr as $val){
						$content[] = array("Title"=>$val['title'], "Description"=>$val['description'], "PicUrl"=>$val['picurl'], "Url" =>$val['url']);
					}
				}
