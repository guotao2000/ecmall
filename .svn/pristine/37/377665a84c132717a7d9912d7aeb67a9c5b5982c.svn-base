...
$notify_url      = "http://ailimo.taobao.com/index.php?app=my_money&act=notify_url";    //交易过程中服务器通知的页面 要用 http://格式的完整路径，不允许加?id=123这类自定义参数
$return_url      = "http://ailimo.taobao.com/index.php?app=my_money&act=return_url";    //付完款后跳转的页面 要用 http://格式的完整路径，不允许加?id=123这类自定义参数
$show_url        = "http://ailimo.taobao.com/index.php?app=my_money&act=paylist";			   //网站商品的展示地址，不允许加?id=123这类自定义参数
