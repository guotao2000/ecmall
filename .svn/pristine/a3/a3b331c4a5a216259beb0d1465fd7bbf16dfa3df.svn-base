	...
	function edit()
	{
		...
		for($i=0;$i<count($stores);$i++){
			...
			if($count = $db->getone($sql1)){
				$sql="update ecm_goods a left join ecm_goods_spec b on a.goods_id = b.goods_id  set a.tags='{$tags}',a.goods_name='{$goods_name}',a.if_show={$if_show},b.sku='{$new_sn}' where b.sku = '{$sn}' and a.store_id = {$stores[$i]}";
				$num = $db->query($sql);
				$storeid .= $stores[$i].",";
			...
			$this->show_message($content,
				'back_list',        'index.php?app=show&goods_sn='.$new_sn	
				);
		}
