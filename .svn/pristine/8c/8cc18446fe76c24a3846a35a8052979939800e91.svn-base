...
class wechatCallbackapiTest 
{
        ...
        switch ($object->Event)
        {
            case "subscribe":
				...
				if(!empty($object->EventKey)){
					...
					$content .= '-------' . $this -> addIntoMemberTable($object->FromUserName, $uin);
				}

				//扫公众号进来的
				if(empty($object->EventKey) || !isset($object->EventKey)){
					//若为-1，则是扫描不带参数公众号进来的
					$content .= '-------' . $this -> addIntoMemberTable($object->FromUserName, -1);
				}
		...
		if(!empty($appid) && !empty($appsecret)){
			...
			if(isset($access_token) && !empty($access_token)){
				...
				if(count($new_array) == 0){
					...
					$sqlzy = "select * from ecm_member where user_name='" . $user_info_arr['openid'] . "'";
					$res_array_xin = mysql_query($sqlzy);
					while ($row = mysql_fetch_array($res_array_xin)) {
						$new_array_r = $row;
					}
					
					//自动登录
					$loginer = new FrontendApp();
					$loginer->_do_login($new_array_r['user_id']);
				...
				if($uin == -1)
				{
					$sql5 = "insert into ecm_member(user_name, password, gender, reg_time, parentid, from_weixin, wx_openid, wx_nickname, wx_city, wx_country, wx_province, wx_language, wx_headimgurl, wx_subscribe_time, wx_id) values('" . $user_info_arr['openid'] . "', '" . md5($user_info_arr['openid']) . "', " . intval($user_info_arr['sex']) . ", " . time() . ", " . $uin . ", 1, '" . $user_info_arr['openid'] . "', '" . $user_info_arr['nickname'] . "', '" . $user_info_arr['city'] . "', '" . $user_info_arr['country'] . "', '" . $user_info_arr['province'] . "', '" . $user_info_arr['language'] . "', '" . $user_info_arr['headimgurl'] . "', " . $user_info_arr['subscribe_time'] . ", " . $wx_id . ")";
					//检测用户openid的唯一性
					$sql4 = "select * from ecm_member where user_name='" . $user_info_arr['openid'] . "'";
					$res_array = mysql_query($sql4);
    ...
    private function receiveText($object)
    {
		...
		$content_text = keywordsArray($keyword);
		return $this->transmitText($object, $content_text);
    }
