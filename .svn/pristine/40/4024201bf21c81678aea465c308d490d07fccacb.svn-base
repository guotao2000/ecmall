		...
		if(!empty($appid) && !empty($appsecret)){
			...
			if(isset($access_token) && !empty($access_token)){
				...
				if(count($new_array) == 0){
					mysql_query($sql3);
					//给该用户发放红包
					$results = mysql_query("SELECT a.*,b.start_time,b.end_time from ecm_coupon_sn a left join ecm_coupon b on a.coupon_id=b.coupon_id where b.end_time>unix_timestamp() and b.if_issue=1 and b.coupon_name='注册专用红包' and a.remain_times > 0 order by a.coupon_id desc");
