...
class BaseOrder extends Object
{
    ...
    function _handle_consignee_info($goods_info, $post)
    {
		...
		$db=&db();
		$sql_address="select * from ecm_address where addr_id=".$post['addressid'];
		$row_addr=$db->getRow($sql_address);
        $consignee_info = $this->_valid_consignee_info(array_merge($post,$row_addr));
        ...
        $shipping_fee = isset($_SESSION['shipping'])?$_SESSION['shipping']:0;//$shipping_info['first_price'] + ($goods_info['quantity'] - 1) * $shipping_info['step_price'];
