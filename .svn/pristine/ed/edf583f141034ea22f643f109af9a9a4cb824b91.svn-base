...
define('NUM_PER_PAGE_WAP', 10);        // 每页显示数量
...
class SearchApp extends MallbaseApp
{
    ...
    function index()
    {
		...
		if (empty($param))
        {
            header('Location: index.php?app=category&id=' . $id);
            exit;
        }
		...
		if(confirm_src())
		{
			 $page = $this->_get_page( NUM_PER_PAGE_WAP);//NUM_PER_PAGE);
		}else
		{
			 $page = $this->_get_page(NUM_PER_PAGE);
		}
        ...
        /* 商品列表 */
		$conditions = $this->_get_goods_conditions_new($param, $id);
		...
		if(!$goods_list)
		{
			...
			$goods_list = $goods_mod->get_list(array(
            	...
            	'limit'      => confirm_src()?NUM_PER_PAGE_WAP:NUM_PER_PAGE,
        	));
     ...
     */
	function _get_goods_conditions_new($param, $store_id)
    {
        /* 组成查询条件 */
		...
		if(isset($store_id))
		{
			$sstore_id='AND g.store_id='.$store_id;
		}
		else{
			$sstore_id='';
		}
		
		$conditions.=$sstore_id ;
