...
class GoodsApp extends StorebaseApp
{
    ...
    function _get_common_info($id)
    {
        ...
        if ($data === false)
        {
			...
			$db = &db();
			$sql_str = "select a.*, c.price from ecm_kuaixun_promotion a LEFT JOIN ecm_goods b on a.goods_id = b.goods_id LEFT JOIN ecm_goods_spec c on a.goods_id=c.goods_id where b.goods_id=" . $goods['goods_id'] . " order by a.add_time desc";
